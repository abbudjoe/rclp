...
|---|---|---|---|---|---|
| ROS 2 robot | Beside the ROS 2 graph or as a robot-local edge authority gate before selected actions, services, or command topics. | Fleet service, remote-assist service, autonomy module, or operator-session controller. | `remote_assist`, operator velocity commands, bounded recovery actions, or temporary speed-envelope changes. | Subscribe to or mirror selected command intents, evaluate allow/deny/degrade, and write audit records without blocking ROS 2 traffic. | ROS 2, DDS, SROS2, node graph design, action semantics, or robot safety controllers. |
...
- Where could a robot-local edge authority gate observe selected command intent
