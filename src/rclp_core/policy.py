...
class RequestReplayCache:
    """In-memory replay window for MVP request nonce checks."""
...
class NetworkRequirements(BaseModel):
    max_latency_ms_p95: float = Field(default=80.0, ge=0)
    max_packet_loss_pct: float = Field(default=1.0, ge=0, le=100)
    min_uplink_mbps: float = Field(default=3.0, ge=0)
    deny_above_latency_ms_p95: float = Field(default=250.0, ge=0)
    deny_above_packet_loss_pct: float = Field(default=10.0, ge=0, le=100)
    deny_below_uplink_mbps: float = Field(default=0.8, ge=0)
